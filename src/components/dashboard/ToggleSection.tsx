...
export const ToggleSection = ({ 
  ...
  const getBorderClass = (isSelected: boolean, itemVariant: string) => {
    ...
    if (itemVariant === "aurora") return "border";
    ...
    <div className={containerClass}>
      {displayToggles.map((item) => {
        ...
        return (
          ...
          >
            <span className={`text-sm ${isAurora ? 'text-text flex-nowrap whitespace-nowrap pr-6 font-medium' : 'text-text flex flex-col items-start'}`}>
              {isAurora 
                ? item.label 

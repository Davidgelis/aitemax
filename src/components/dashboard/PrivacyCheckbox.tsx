  ...
  return (
    <div className={`relative flex items-center ${className}`}>
      <button 
        ...
        className="h-10 w-36 bg-white border border-[#e5e7eb] text-[#545454] hover:bg-[#f8f9fa] flex justify-between items-center shadow-sm text-sm rounded-md px-4"
      >
        <span className="whitespace-nowrap">{isPrivate ? 'Privacy On' : 'Privacy'}</span>
        
        <div className="flex items-center">
          {isPrivate ? (
            <span className="h-4 w-4 flex items-center justify-center mr-1.5">
              <svg 
                width="14" 
                height="14" 
                viewBox="0 0 24 24" 
            ...
            <Checkbox 
              ...
              className="data-[state=checked]:bg-transparent data-[state=checked]:border-[#e5e7eb] mr-1.5"
            />
          )}
